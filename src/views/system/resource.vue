	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="searchForm">
				...
				<el-form-item>
					<el-button type="primary" v-on:click="getResource">查询</el-button>
				</el-form-item>
				<el-form-item>
	...
	export default {
		...
		methods: {
            ...
            getResource() {
                ...
                this.$http.patch("/resource/selectPageByQuery",param).then(res=>{
                    console.debug(res);
            ...
            submit() {
                ...
                this.$refs.addForm.validate((valid) => {
                    if (valid) {//校验通过之后就进入该代码块
                        ...
                        if (param.id) {
                            //保存数据
                            this.$http.post("/resource/update", param).then(res => {
                                //提交成功
                                if (res.data.success) {
                                    //停止添加按钮的加载框
                            ...
                            //保存数据
                            this.$http.put("/resource/save", param).then(res => {
                                //提交成功
                                if (res.data.success) {
                                    //停止添加按钮的加载框
                                    this.addLoading = false;
            ...
            handleDel(i,r){
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.$http.delete("/resource/delete/"+r.id).then(res=>{
                        if(res.data.success){
                            this.$message({
                                message: '删除成功',
                                type: 'success'
            ...
            batchRemove(){
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.$http.patch("/resource/batchDelete",this.sels).then(res=>{
                        if(res.data.success){
                            //NProgress.done();
                            this.$message({
                                message: '删除成功',
